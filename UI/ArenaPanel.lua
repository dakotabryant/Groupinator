...
local ArenaPanel = CreateFrame("Frame", "GroupinatorArenaPanel", GRP.Dialog, "GroupinatorArenaPanelTemplate")

function ArenaPanel:OnLoad()
    GRP.Logger:Debug("ArenaPanel:OnLoad")
    self.name = "arena"

    -- Group
    self.Group.Title:SetText(L["dialog.filters.group"])

    GRP.UI_SetupDropDown(self, self.Group.Difficulty, "ArenaDifficultyMenu", L["dialog.difficulty"], DIFFICULTY_TEXT)
    GRP.UI_SetupMinMaxField(self, self.Group.PvPRating, "pvprating")
...
function ArenaPanel:Init(state)
    GRP.Logger:Debug("Arenapanel:Init")
    self.state = state
...
function ArenaPanel:OnShow()
    GRP.Logger:Debug("ArenaPanel:OnShow")
end

function ArenaPanel:OnHide()
    GRP.Logger:Debug("ArenaPanel:OnHide")
end

function ArenaPanel:OnReset()
    GRP.Logger:Debug("ArenaPanel:OnReset")
...
function ArenaPanel:OnUpdateExpression(expression, sorting)
    GRP.Logger:Debug("ArenaPanel:OnUpdateExpression")
    self.state.expression = expression
    self:Init(self.state)
end

function ArenaPanel:TriggerFilterExpressionChange()
    GRP.Logger:Debug("ArenaPanel:TriggerFilterExpressionChange")
...
function ArenaPanel:GetFilterExpression()
    GRP.Logger:Debug("ArenaPanel:GetFilterExpression")
    ...
    if self.state.pvprating.act then
        if GRP.NotEmpty(self.state.pvprating.min) then expression = expression .. " and pvprating >= " .. self.state.pvprating.min end
        if GRP.NotEmpty(self.state.pvprating.max) then expression = expression .. " and pvprating <= " .. self.state.pvprating.max end
    end
...
ArenaPanel:OnLoad()
GRP.Dialog:RegisterPanel("c4f8", ArenaPanel)
