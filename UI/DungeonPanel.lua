...
local DungeonPanel = CreateFrame("Frame", "GroupinatorDungeonPanel", GRP.Dialog, "GroupinatorDungeonPanelTemplate")

function DungeonPanel:OnLoad()
    GRP.Logger:Debug("DungeonPanel:OnLoad")
    self.name = "dungeon"
    ...
    GRP.UI_SetupDropDown(self, self.Group.Difficulty, "DungeonDifficultyMenu", L["dialog.difficulty"], DIFFICULTY_TEXT, self.groupWidth)
    if self.Group.Playstyle then
        GRP.UI_SetupDropDown(self, self.Group.Playstyle, "DungeonPlaystyleMenu", L["dialog.playstyle"] or "Playstyle", PLAYSTYLE_TEXT, self.groupWidth)
    end
    GRP.UI_SetupMinMaxField(self, self.Group.MPRating, "mprating", self.groupWidth)
    GRP.UI_SetupMinMaxField(self, self.Group.Members, "members", self.groupWidth)
    GRP.UI_SetupMinMaxField(self, self.Group.Tanks, "tanks", self.groupWidth)
    GRP.UI_SetupMinMaxField(self, self.Group.Heals, "heals", self.groupWidth)
    GRP.UI_SetupMinMaxField(self, self.Group.DPS, "dps", self.groupWidth)
    GRP.UI_SetupCheckBox(self, self.Group.Partyfit, "partyfit", self.groupWidth)
    GRP.UI_SetupCheckBox(self, self.Group.BLFit, "blfit", self.groupWidth)
    GRP.UI_SetupCheckBox(self, self.Group.NeedsBL, "needsbl", self.groupWidth)
    GRP.UI_SetupCheckBox(self, self.Group.NotDeclined, "notdeclined", self.groupWidth)
...
function DungeonPanel:InitChallengeModes()
    GRP.Logger:Debug("Dungeonpanel:InitChallengeModes")
    if not C_ChallengeMode.GetMapTable() then
        GRP.Logger:Debug("C_ChallengeMode.GetMapTable() not yet ready")
        return
    end
...
function DungeonPanel:Init(state)
    GRP.Logger:Debug("Dungeonpanel:Init")
    self.state = state
    self.state.difficulty = self.state.difficulty or {}
    self.state.playstyle = self.state.playstyle or {}
    ...
    if self.Group.Playstyle then
        self.Group.Playstyle.Act:SetChecked(self.state.playstyle.act or false)
        self.Group.Playstyle.DropDown:SetKey(self.state.playstyle.val)
    end
...
function DungeonPanel:OnEvent(event)
    if event == "CHALLENGE_MODE_MAPS_UPDATE" then
        GRP.Logger:Debug("DungeonPanel:OnEvent(CHALLENGE_MODE_MAPS_UPDATE)")
        self:InitChallengeModes()
    elseif (event == "ACTIVE_PLAYER_SPECIALIZATION_CHANGED" or event == "GROUP_ROSTER_UPDATE") and self.state then
        GRP.Logger:Debug("DungeonPanel:OnEvent(" .. event .. ")")
...
function DungeonPanel:OnShow()
    GRP.Logger:Debug("DungeonPanel:OnShow")
    self:TryInitChallengeModes()
    self:UpdateCheckboxVisibility()
end

function DungeonPanel:OnHide()
    GRP.Logger:Debug("DungeonPanel:OnHide")
end

function DungeonPanel:OnReset()
    GRP.Logger:Debug("DungeonPanel:OnReset")
    self.state.difficulty.act = false
    self.state.playstyle.act = false
...
function DungeonPanel:OnUpdateExpression(expression, sorting)
    GRP.Logger:Debug("DungeonPanel:OnUpdateExpression")
    self.state.expression = expression
    self:Init(self.state)
end

function DungeonPanel:TriggerFilterExpressionChange()
    GRP.Logger:Debug("DungeonPanel:TriggerFilterExpressionChange")
...
function DungeonPanel:GetFilterExpression()
    GRP.Logger:Debug("DungeonPanel:GetFilterExpression")
    ...
    if self.state.playstyle and self.state.playstyle.act and self.state.playstyle.val then
        local keyword = C.PLAYSTYLE_KEYWORD[self.state.playstyle.val]
        if keyword then expression = expression .. " and " .. keyword end
    end
    if self.state.mprating.act then
        if GRP.NotEmpty(self.state.mprating.min) then expression = expression .. " and mprating >= " .. self.state.mprating.min end
        if GRP.NotEmpty(self.state.mprating.max) then expression = expression .. " and mprating <= " .. self.state.mprating.max end
    end
    if self.state.members.act then
        if GRP.NotEmpty(self.state.members.min) then expression = expression .. " and members >= " .. self.state.members.min end
        if GRP.NotEmpty(self.state.members.max) then expression = expression .. " and members <= " .. self.state.members.max end
    end
    if self.state.tanks.act then
        if GRP.NotEmpty(self.state.tanks.min) then expression = expression .. " and tanks >= " .. self.state.tanks.min end
        if GRP.NotEmpty(self.state.tanks.max) then expression = expression .. " and tanks <= " .. self.state.tanks.max end
    end
    if self.state.heals.act then
        if GRP.NotEmpty(self.state.heals.min) then expression = expression .. " and heals >= " .. self.state.heals.min end
        if GRP.NotEmpty(self.state.heals.max) then expression = expression .. " and heals <= " .. self.state.heals.max end
    end
    if self.state.dps.act then
        if GRP.NotEmpty(self.state.dps.min) then expression = expression .. " and dps >= " .. self.state.dps.min end
        if GRP.NotEmpty(self.state.dps.max) then expression = expression .. " and dps <= " .. self.state.dps.max end
    end
    if self.state.partyfit then
        expression = expression .. " and partyfit"
    end
    if self.state.blfit and not GRP.PlayerOrGroupHasBloodlust() then -- use blfit only if group does not have BL
        expression = expression .. " and blfit"
    end
    if self.state.needsbl and GRP.PlayerOrGroupHasBloodlust() then -- use needsbl only if group has BL
...
function DungeonPanel:UpdateCheckboxVisibility()
    if GRP.PlayerOrGroupHasBloodlust() then
...
function DungeonPanel:UpdateAdvancedFilters()
    local enabled = GRP.GetAdvancedFilterDefaults()
    if self.state.difficulty.act and self.state.difficulty.val then
    ...
    if self.state.mprating.act then
        enabled.minimumRating = GRP.NotEmpty(self.state.mprating.min) and tonumber(self.state.mprating.min) or 0
        --MinRatingFrame.MinRating:SetNumber(enabled.minimumRating)
    end
    if self.state.tanks.act then
        enabled.hasTank = GRP.NotEmpty(self.state.tanks.min) and tonumber(self.state.tanks.min) > 0
        enabled.needsTank = GRP.NotEmpty(self.state.tanks.max) and tonumber(self.state.tanks.max) == 0
    end
    if self.state.heals.act then
        enabled.hasHealer = GRP.NotEmpty(self.state.heals.min) and tonumber(self.state.heals.min) > 0
        enabled.needsHealer = GRP.NotEmpty(self.state.heals.max) and tonumber(self.state.heals.max) == 0
    end
    if self.state.dps.act then
        enabled.needsDamage = GRP.NotEmpty(self.state.dps.max) and tonumber(self.state.dps.max) < 3
    end
    if self.state.partyfit then
    ...
    GRP.SetAdvancedFilter(enabled)
end

DungeonPanel:OnLoad()
GRP.Dialog:RegisterPanel("c2f4", DungeonPanel)
