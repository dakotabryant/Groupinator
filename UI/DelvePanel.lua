...
local DelvePanel = CreateFrame("Frame", "GroupinatorDelvePanel", GRP.Dialog, "GroupinatorDelvePanelTemplate")
...
function DelvePanel:OnLoad()
    GRP.Logger:Debug("DelvePanel:OnLoad")
    self.name = "delve"
    ...
    GRP.UI_SetupMinMaxField(self, self.Group.DelveTier, "delvetier", self.groupWidth)
    ...
    for i = 1, NUM_DELVE_CHECKBOXES do
        ...
        local name = GRP.String_RemoveBrackets(activityInfo.fullName)
...
function DelvePanel:Init(state)
    GRP.Logger:Debug("DelvePanel:Init")
    self.state = state
...
function DelvePanel:UpdateDelves()
    ...
    for i = 1, NUM_DELVE_CHECKBOXES do
        ...
        for _, bountifulDelveName in ipairs(bountifulDelves) do
            if GRP.IsMostLikelySameInstance(delve.name, bountifulDelveName) then
...
function DelvePanel:OnEvent(event)
    if event == "AREA_POIS_UPDATED" then
        GRP.Logger:Debug("DungeonPanel:OnEvent(AREA_POIS_UPDATED)")
        self:UpdateDelves()
    end
end

function DelvePanel:OnShow()
    GRP.Logger:Debug("DelvePanel:OnShow")
    self:UpdateDelves()
end

function DelvePanel:OnHide()
    GRP.Logger:Debug("DelvePanel:OnHide")
end

function DelvePanel:OnReset()
    GRP.Logger:Debug("DelvePanel:OnReset")
...
function DelvePanel:OnUpdateExpression(expression, sorting)
    GRP.Logger:Debug("DelvePanel:OnUpdateExpression")
    self.state.expression = expression
    self:Init(self.state)
end

function DelvePanel:TriggerFilterExpressionChange()
    GRP.Logger:Debug("DelvePanel:TriggerFilterExpressionChange")
...
DelvePanel:OnLoad()
GRP.Dialog:RegisterPanel("c121f4", DelvePanel)
