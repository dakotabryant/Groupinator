...
function GRP.PopupMenu_Register(name, entries, parent, anchorPoint, relativeTo, relativePoint, offsetX, offsetY, minWidth, maxWidth, fontSize)
    popupMenus[name] = {
        entries = entries,
...
function GRP.PopupMenu_Initialize(name)
    local menu = popupMenus[name]

    -- create frame if it does not yet exist
    if not popupFrame then
        popupFrame = CreateFrame("Frame", nil, nil, "GroupinatorPopupMenuFrameTemplate")
    ...
    for i, entry in ipairs(menu.entries) do
        ...
        if not button then
            popupFrame.Buttons[i] = CreateFrame("Button", nil, popupFrame, "GroupinatorPopupMenuButtonTemplate")
...
function GRP.PopupMenu_Show(name)
    ...
    GRP.PopupMenu_Initialize(name)
    popupFrame:Show()
end

function GRP.PopupMenu_Hide()
    popupMenuActive = nil
    if popupFrame then popupFrame:Hide() end
end

function GRP.PopupMenu_Toggle(name)
    if popupMenuActive == name then GRP.PopupMenu_Hide(name) else GRP.PopupMenu_Show(name) end
end

function GRP.Popup_GetSelectedEntry()
