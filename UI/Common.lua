...
function GRP.UI_SetupCheckBox(panel, field, keyword, size)
...
function GRP.UI_SetupMinMaxField(panel, field, keyword, size)
    ...
    local autoCheckbox = function ()
        local shouldCheck = GRP.NotEmpty(field.Min:GetText()) or GRP.NotEmpty(field.Max:GetText())
...
function GRP.UI_SetupDropDown(panel, field, name, title, entryTable, size)
    ...
    GRP.PopupMenu_Register(name, entries, field:GetParent(), "TOPRIGHT", dropdown, "BOTTOMRIGHT", -15, 10, 95, 150)
    dropdown.Button:SetScript("OnClick", function () GRP.PopupMenu_Toggle(name) end)
    dropdown:SetScript("OnHide", GRP.PopupMenu_Hide)
end


local advancedExpressionPanels = {}

function GRP.UI_SetupAdvancedExpression(panel)
    ...
    panel.Advanced.Info:SetScript("OnEnter", GRP.Dialog_InfoButton_OnEnter)
    panel.Advanced.Info:SetScript("OnLeave", GRP.Dialog_InfoButton_OnLeave)
    panel.Advanced.Info:SetScript("OnClick", GRP.Dialog_InfoButton_OnClick)
end

--- Removes all comment lines (--), trims each line and joins them to one single line
function GRP.UI_NormalizeExpression(exp)
