...
GRP.roleIndicators = {}
function GRP.GetOrCreateRoleIndicatorFrames(self, numIcons)
    ...
    local frames = GRP.roleIndicators[self]
    if frames == nil then
        frames = {}
        ...
        GRP.roleIndicators[self] = frames
    end
    return frames
end

function GRP.AddRoleIndicators(self, searchResultInfo)
    local numIcons = #self.DataDisplay.Enumerate.Icons -- should always be 5 for Enum.LFGListDisplayType.RoleEnumerate
    local frames = GRP.GetOrCreateRoleIndicatorFrames(self, numIcons)
    ...
    if not GroupinatorSettings.classBar and
       not GroupinatorSettings.classCircle and
       not GroupinatorSettings.specIcon and
       not GroupinatorSettings.leaderCrown and
       not GroupinatorSettings.missingRoles then
        return -- stop if all features are disabled
    end

    local appStatus, isApplication, isDeclined = GRP.GetAppStatus(self.resultID, searchResultInfo)
    ...
    for i = 1, #members do
        ...
        if GroupinatorSettings.specIcon then
        ...
        if GroupinatorSettings.leaderCrown and members[i].isLeader then
    ...
    if GroupinatorSettings.missingRoles then
        local roleAtlas = GRP.IsRetail() and C.ROLE_ATLAS_BORDERLESS or C.ROLE_ATLAS
        local i = #members + 1
