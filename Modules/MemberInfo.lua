...
function GRP.PutSearchResultMemberInfos(resultID, searchResultInfo, env)
    -- init to zero
    env.ranged = 0
    env.melees = 0
    ...
    env.hasleaver = false
    local mySpecInfo = GRP.GetSpecializationInfoForPlayer()
    local specs = GRP.GetAllSpecializations()
    ...
    for i = 1, searchResultInfo.numMembers do
        local role, class, classLocalized, specLocalized, isLeader, isLeaver = GRP.GetSearchResultMemberInfo(resultID, i)
        if isLeaver then
            env.hasleaver = true
        end
        local specInfo = GRP.GetSpecializationInfoByLocalizedName(class, specLocalized)
        if specInfo then
...
local function GetRoleClassOrder(resultID)
    ...
    for i = 1, #roleOrder do
        ...
        if displayData.classesByRole then
            ...
            result[role] = GRP.Table_Invert(CLASS_SORT_ORDER)
        end
    end
...
function GRP.GetSearchResultMemberInfoTable(resultID, numMembers)
    local members = {}
    for i = 1, numMembers do
        local role, class, classLocalized, specLocalized, isLeader, isLeaver = GRP.GetSearchResultMemberInfo(resultID, i)
        local specInfo = GRP.GetSpecializationInfoByLocalizedName(class, specLocalized)
        if specInfo then
            local memberInfo = GRP.Table_Copy_Shallow(specInfo)
