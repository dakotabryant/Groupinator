...
GRP.clickToCancelFrames = {}

hooksecurefunc("LFGListSearchEntry_OnEnter", function (self)
    if not GroupinatorSettings.cancelOldestApp then return end

    local clickToCancelFrame = GRP.clickToCancelFrames[self]
    if clickToCancelFrame == nil then
        ...
        GRP.clickToCancelFrames[self] = frame
...
hooksecurefunc("LFGListSearchEntry_OnClick", function (self, button)
    ...
    if GroupinatorSettings.cancelOldestApp and button ~= "RightButton" then
        ...
        if ShouldCancelApplication(self.resultID) then
            ...
            if oldestResultID > 0 then
                GRP.Logger:Debug("Canceling application "..oldestResultID)
    ...
    if GroupinatorSettings.oneClickSignUp and button ~= "RightButton"
...
LFGListApplicationDialog:HookScript("OnShow", function(self)
    if not GroupinatorSettings.skipSignUpDialog then return end
