...
function GRP.GetPvPScoreRarityColorByTier(tier)
    local r, g, b = C_Item.GetItemQualityColor(C.PVP_TIER_MAP[tier].quality)
    return { r = r, g = g, b = b }
end

GRP.ratingInfoFrames = {}
function GRP.GetOrCreateRatingInfoFrame(self)
    ...
    local frame = GRP.ratingInfoFrames[self]
    if frame == nil then
        ...
        GRP.ratingInfoFrames[self] = frame
    end
    return frame
end

function GRP.AddRatingInfo(self, searchResultInfo)
    local frame = GRP.GetOrCreateRatingInfoFrame(self)
    ...
    if not GroupinatorSettings.ratingInfo then
    ...
    if GRP.SupportsMythicPlus() and activityInfo.isMythicPlusActivity then
        rightPos = -115
        rating = searchResultInfo.leaderOverallDungeonScore or 0
        if GroupinatorSettings.rioRatingColors and RaiderIO and RaiderIO.GetScoreColor then
    ...
    if activityInfo.isRatedPvpActivity then
        ...
        if searchResultInfo.leaderPvpRatingInfo then
            ...
            ratingColor = GRP.GetPvPScoreRarityColorByTier(searchResultInfo.leaderPvpRatingInfo.tier or 0) or ratingColor
        end
    end
