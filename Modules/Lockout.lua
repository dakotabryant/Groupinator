...
function GRP.IsMatchingInstance(lockoutName, activityName, lockoutDifficulty, activityDifficulty)
    -- no match if difficulty does not match
    if not (C.DIFFICULTY_MAP[lockoutDifficulty] == activityDifficulty) then return false end
    return GRP.IsMostLikelySameInstance(lockoutName, activityName)
end

function GRP.GetLockoutInfo(activity, resultID)
    ...
    local groupDefeatedBossNames = GRP.Table_ValuesAsKeys(encounterInfo)
    local numGroupDefeated = GRP.Table_Count(encounterInfo)
    ...
    for index = 1, numSavedInstances do
        ...
        if (extended or locked) and GRP.IsMatchingInstance(instanceName, activityInfo.fullName, instanceDifficulty, difficulty) then
            local playerDefeatedBossNames = GRP.GetPlayerDefeatedBossNames(index, maxBosses)
            local numPlayerDefeated = GRP.Table_Count(playerDefeatedBossNames)
            local matching, groupAhead, groupBehind = GRP.GetMatchingBossInfo(groupDefeatedBossNames, playerDefeatedBossNames)
...
function GRP.GetMatchingBossInfo(groupDefeatedBossNames, playerDefeatedBossesNames)
    local matching = 0
    local groupAhead = 0
    local groupBehind = 0
...
function GRP.GetPlayerDefeatedBossNames(savedInstanceIndex, maxBosses)
    local result = {}
