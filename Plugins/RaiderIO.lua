...
function GRP.GetNameRealmFaction(leaderName)
...
function GRP.PutRaiderIOMetrics(env, leaderName, activityID)
    env.hasrio            = false
    env.norio             = true
    env.rio               = 0
    env.rioprev           = 0
    ...
    if leaderName and RaiderIO and RaiderIO.GetProfile then
        -- new API
        local name, realm = GRP.GetNameRealmFaction(leaderName)
