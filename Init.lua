...
Groupinator = {}
GroupinatorSettings = GroupinatorSettings or {}

local GRPAddonName = select(1, ...)
local GRP = select(2, ...)

Groupinator.Debug = GRP
...
function GRP.IsRetail() return true end
function GRP.SupportsMythicPlus() return true end
function GRP.SupportsSpecializations() return true end
function GRP.SupportsDragonflightUI() return true end
...
function GRP.MigrateStateV4()
    if GroupinatorState.version < 4 then
        for k, v in pairs(GroupinatorState) do
            if type(v) == "table" then
                v.expert = GroupinatorState.expert
            end
        end
        GroupinatorState.expert = nil
        GroupinatorState.version = 4
        print(string.format(L["message.settingsupgraded"], "4"))
    end
end

function GRP.MigrateStateV5()
    if GroupinatorState.version < 5 then
        local mapping = {
        ...
        GroupinatorState = state5
        print(string.format(L["message.settingsupgraded"], "5"))
    end
end

function GRP.MigrateStateV6()
    if GroupinatorState.version < 6 then
        for k, v in pairs(GroupinatorState) do
        ...
        GroupinatorState.version = 6
        print(string.format(L["message.settingsupgraded"], "6"))
    end
end

function GRP.MigrateStateV7()
    if GroupinatorState.version < 7 then
        if GroupinatorState.c2f4 and GroupinatorState.c2f4.dungeon then
            GroupinatorState.c2f4.dungeon.blfit = nil
            GroupinatorState.c2f4.dungeon.brfit = nil
        end
        GroupinatorState.version = 7
        print(string.format(L["message.settingsupgraded"], "7"))
    end
end

function GRP.MigrateStateV8()
    if GroupinatorState.version < 8 then
        -- Add delves support
        if not GroupinatorState.c121f4 then
            GroupinatorState.c121f4 = { enabled = true }
        end
        GroupinatorState.version = 8
        print(string.format(L["message.settingsupgraded"], "8"))
    end
end

function GRP.MigrateSettingsV2()
    if not GroupinatorSettings.version or GroupinatorSettings.version < 2 then
        if GRP.IsRetail() then -- disable features now provided by default
            GroupinatorSettings.classCircle = false
        end
        GroupinatorSettings.version = 2
    end
end

function GRP.MigrateSettingsV3()
    if not GroupinatorSettings.version or GroupinatorSettings.version < 3 then
        GroupinatorSettings.coloredApplications = nil
        GroupinatorSettings.version = 3
    end
end

function GRP.OnAddonLoaded(name)
    if name == GRPAddonName then
        -- update new settings with defaults
        GRP.Table_UpdateWithDefaults(GroupinatorSettings, GRP.C.SETTINGS_DEFAULT)
        GRP.MigrateSettingsV2()
        GRP.MigrateSettingsV3()

        -- initialize dialog state and migrate to latest version
        if GroupinatorState == nil or GroupinatorState.version == nil then
            GroupinatorState = {
                version = 8,
        ...
        GRP.InitSpecializations()
    end
end

function GRP.OnPlayerLogin()
    GRP.PersistSignUpNote()
end

function GRP.OnEvent(self, event, ...)
    if event == "ADDON_LOADED" then GRP.OnAddonLoaded(...) end
    if event == "PLAYER_LOGIN" then GRP.OnPlayerLogin() end
    if event == "LFG_LIST_APPLICATION_STATUS_UPDATED" then GRP.OnLFGListApplicationStatusUpdated(...) end
end

local frame = CreateFrame("Frame", "GroupinatorEventFrame")
frame:RegisterEvent("ADDON_LOADED")
frame:RegisterEvent("PLAYER_LOGIN")
frame:RegisterEvent("LFG_LIST_APPLICATION_STATUS_UPDATED")
frame:SetScript("OnEvent", GRP.OnEvent)
